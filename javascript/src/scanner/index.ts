...
export class Scanner {
    ...
    identifierKind(): TokenKind {
        if (this.current - this.start === 1) {
            ...
            switch(this.source[this.start]) {
                case 'b': switch(this.source[this.start + 1]) {
                    case 'l': switch(this.source[this.start + 2]) {
                        case 'a': return this.checkKeyword(3, 2, "ck", TokenKind.Black)
                        case 'u': return this.checkKeyword(3, 1, "e", TokenKind.Blue)
                        default: return TokenKind.Identifier
                    }
                }
                case 'c': return this.checkKeyword(1, 3, "yan", TokenKind.Cyan)
                case 'g': return this.checkKeyword(1, 4, "reen", TokenKind.Green)
                case 'm': return this.checkKeyword(1, 6, "agenta", TokenKind.Magenta)
                case 'r': switch(this.source[this.start + 1]) {
                    case 'e': return this.checkKeyword(2, 1, "d", TokenKind.Red)
                    case 'g': return this.checkKeyword(2, 1, "b", TokenKind.Rgb)
                    default: return TokenKind.Identifier
                }
                case 'w': return this.checkKeyword(1, 4, "hite", TokenKind.White)
                case 'y': return this.checkKeyword(1, 5, "ellow", TokenKind.Yellow)
                default: return TokenKind.Identifier
            }
        }
    }
    ...
    scanToken(): Token {
        ...
        switch(c) {
            case '(': return this.makeToken(TokenKind.LeftParen)
            case ')': return this.makeToken(TokenKind.RightParen)
            case ',': return this.makeToken(TokenKind.Comma)
            case '.': return this.makeToken(TokenKind.Dot)
            case '/': return this.makeToken(TokenKind.Slash)
            default: return this.errorToken(1)
        }
    }
}